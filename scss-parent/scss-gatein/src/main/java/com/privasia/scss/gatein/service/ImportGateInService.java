...
import com.privasia.scss.core.service.GatePassService;
...
public class ImportGateInService {
	...
	public GatePassValidateDTO validateGatePass(Long cardIdSeq, Long gatePassNo, boolean checkPreArrival,
		...
		log.error("------Gate Pass Validate Inputs ------" + " gatePassNo :" + gatePassNo + " truckHeadNo :"
		...
		if (!(gatePass.getCommonGateInOut() == null
			...
			if (gatePass.getBaseCommonGateInOutAttribute().getEirStatus().equals(eirStatus)) {
				 ...
				 * CommonUtil.formatMessageCode(ApplicationConstants.
	        ...
	        WDCGatePass wdcGatePass = wdcGatePassOpt.orElseThrow(() -> new BusinessException("Invalid Gate Pass In WDC. Not Found !" + gatePassNo));
					...
					if (!(gatePass == null)) {
						...
						if (!(importContainer.getBaseCommonGateInOutAttribute() == null)) {
							...
							if (!(importContainer.getBaseCommonGateInOutAttribute().getCard() == null)) {
								card = cardRepository
										.findOne(importContainer.getBaseCommonGateInOutAttribute().getCard())
										.orElse(null);
							}
						}
						...
						if (!(importContainer.getBaseCommonGateInOutAttribute() == null
							...
							gateInClient = clientRepository.findOne(
									importContainer.getBaseCommonGateInOutAttribute().getGateInClient().getClientID())
									.orElse(null);
						}
						...
						if (!(importContainer.getPrintEir() == null
							...
							printEir = printEirRepository.findOne(importContainer.getPrintEir().getPrintEIRID())
									.orElse(null);
						}
						...
						if (!(importContainer.getCardUsage() == null
							...
							cardUsage = cardUsageRepository.findOne(importContainer.getCardUsage().getCardUsageID())
									.orElse(null);
						}

						modelMapper.map(importContainer, gatePass);
						gatePass.prepareForInsertFromOpus(card, gateInClerk, gateInClient, printEir, cardUsage,
								hpatBooking);
