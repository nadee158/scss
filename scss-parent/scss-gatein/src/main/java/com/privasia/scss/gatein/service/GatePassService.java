...
public class GatePassService {
		...
		if (scssGatePassOpt.isPresent()) {
			...
			if (!(gatePass.getCommonGateInOut() == null
				...
				if (gatePass.getBaseCommonGateInOutAttribute().getEirStatus().equals(eirStatus)) {
					throw new BusinessException(CommonUtil.formatMessageCode(ApplicationConstants.GATE_PASS_IS_USED,
			...
			if (!(gatePass.getCommonGateInOut() == null
					|| gatePass.getBaseCommonGateInOutAttribute().getEirStatus() == null)) {
				if (gatePass.getBaseCommonGateInOutAttribute().getEirStatus().equals(eirStatus)) {
					throw new BusinessException(CommonUtil.formatMessageCode(ApplicationConstants.GATE_PASS_IN_PROGRESS,
							new Object[] { gatePassNo }));
				}
			}
			...
			if (!(gatePass.getGatePassStatus() == null)) {
				if (gatePass.getGatePassStatus().equals(gatePassStatus)) {
					throw new BusinessException(CommonUtil.formatMessageCode(ApplicationConstants.GATE_PASS_CANCEL,
							new Object[] { gatePassNo }));
				}
				...
				if (!(gatePass.getCommonGateInOut() == null
					...
					if (gatePass.getBaseCommonGateInOutAttribute().getEirStatus().equals(eirStatus)
						...
						throw new BusinessException(CommonUtil.formatMessageCode(ApplicationConstants.GATE_PASS_INVALID,
								new Object[] { gatePassNo }));
					}
				}
				...
				if (!(validateDate == null)) {
					...
					if (today.isAfter(validateDate)) {
						throw new BusinessException(CommonUtil.formatMessageCode(
								ApplicationConstants.DATE_GATEPASS_EXPIRY, new Object[] { gatePassNo }));
					}
					log.error("------END check Gate Pass Expiry Date By YPN? ---- gatePassNo: " + gatePassNo
							+ " :validateDate: " + validateDate + " :today: " + today);
				}
				...
				Optional<WDCGatePass> wdcGatePassOpt = wdcGatePassRepository
						.findByGatePassNO(Long.parseLong(gatePassNo));
				...
				if (wdcGatePassOpt.isPresent()) {
					...
					if (wdcGlobalSettingOpt.isPresent()) {
						...
						if ("Y".equalsIgnoreCase(etpEdoExpiryDateFlag)) {
							...
							log.error(
									"-------------START Edo Expiry Date -----------" + gatePassNo + ":" + truckHeadNo);
								...
								if ("I".equalsIgnoreCase(gateOrderType) || "J".equalsIgnoreCase(gateOrderType)
									...
									if (edoExpiryDate != null) {
										if (today.isAfter(edoExpiryDate)) {
											throw new BusinessException(CommonUtil.formatMessageCode(
													ApplicationConstants.DATE_GATEPASS_EDO_EXPIRY,
													new Object[] { gatePassNo }));
										}
									} else {
										...
										if (StringUtils.isNotBlank(lineCode)) {
											...
											if (responseType.isEdoExpiryEnabled()) {
												if (edoExpiryDate == null) {
													throw new BusinessException(CommonUtil.formatMessageCode(
															ApplicationConstants.EDO_EXPIRY_DATE_NULL,
															new Object[] { gatePassNo }));
												}
											}
										}
							 ...
							 */
							return checkMatchCompanyPreArrival(cardIdSeq, gatePassNo, check, hpatSeqId, truckHeadNo,
									companyId, gatePass);
					...
					if (edoExpiryDate != null) {
						if (LocalDateTime.now().isAfter(edoExpiryDate)) {
							throw new BusinessException(
									"Gate Pass No " + gatePass.getGatePassNo() + " Demurrage already Expiry");

							/*
							 * throw new BusinessException(CommonUtil
							 * .formatMessageCode(ApplicationConstants.
							 * DATE_GATEPASS_EDO_EXPIRY, new Object[]
						...
						if (StringUtils.isNotBlank(lineCode)) {
							...
							if (responseType.isEdoExpiryEnabled()) {
								if (edoExpiryDate == null) {
									 ...
									 * throw new BusinessException(CommonUtil
									 * .formatMessageCode(ApplicationConstants.
									 * EDO_EXPIRY_DATE_NULL, new Object[]
									 * {gatePass.getGatePassNo()}));
									 */
								}
							}
	...
	public void matchCompany(GatePass gatePass, Long cardIdSeq) {
		...
		Card card = cardOpt.orElseThrow(() -> new ResultsNotFoundException("Card could not be Found for given card Id ! " + cardIdSeq));
		...
		if (gatePass.getCompany()!= null ||card.getCompany()!= null ) {
			if(gatePass.getCompany().getCompanyID().longValue() != card.getCompany().getCompanyID().longValue()){
				throw new BusinessException("Company''s Gate Pass No " + gatePass.getGatePassNo() + " and Smart Card No do not match");
			}
	...
	public boolean checkMatchCompanyPreArrival(String cardIdSeq, String gatePassNo, String check, String hpatSeqId,
		...
		if (!(companyId == hcid)) {
			throw new BusinessException(CommonUtil.formatMessageCode(ApplicationConstants.GATE_PASS_COMPANY_NOT_MATCH,
					new Object[] { gatePassNo }));
		}
		...
		if (isOgaBlock && isInternalBlock) {
			throw new BusinessException(CommonUtil.formatMessageCode(ApplicationConstants.GATE_PASS_OGA_INTERNAL_BLOCK,
					new Object[] { containerNo }));
		} else if (isOgaBlock && !isInternalBlock) {
			throw new BusinessException(CommonUtil.formatMessageCode(ApplicationConstants.GATE_PASS_OGA_BLOCK,
					new Object[] { containerNo }));
		} else if (!isOgaBlock && isInternalBlock) {
			throw new BusinessException(CommonUtil.formatMessageCode(ApplicationConstants.GATE_PASS_INTERNAL_BLOCK,
					new Object[] { containerNo }));
		}
						...
						if (company.getCompanyType().equals(CompanyType.HAULAGE)) {
							...
							if (StringUtils.isNotBlank(smartCardUser.getCommonContactAttribute().getNewNRICNO())) {
								icNo = smartCardUser.getCommonContactAttribute().getNewNRICNO();
									...
									.isNotBlank(smartCardUser.getCommonContactAttribute().getOldNRICNO())) {
							...
							Optional<WDCGlobalSetting> wdcGlobalSettingOpt = wdcGlobalSettingRepository
									.findOne("ETP_HPAT");
							if (wdcGlobalSettingOpt.isPresent()) {
								WDCGlobalSetting wdcGlobalSetting = wdcGlobalSettingOpt.get();
	...
	private boolean checkHpatBookingExist(String containerNo, String truckHeadNo, Card card) throws Exception {
		if (!(card == null)) {
			...
			if (StringUtils.isNotBlank(truckHeadNo)) {
				...
				hpatBookingDetail = hpatBookingDetailRepository
						.findByContainerNumberAndBookingTypeAndHpatBooking_StatusAndHpatBooking_CardNo(containerNo,
								type, hpatReferStatus, cardNo);
			}
	...
	public GatePass getGatePassForImportContainer(ImportContainer container, long companyID) {
		if (!(container == null)) {
			...
			// try to find by gatePassNo1
			Optional<GatePass> gatePassOptional = gatePassRepository.findByGatePassNoAndCompany_companyID(gatePassNo,
					companyID);
	...
	public void updateGatePass(TransactionDTO transactionDTO, LocalDateTime timeGateIn, String clientId,
			String cardIdSeq, String expImpFlag) throws Exception {
		...
		if (!(container == null || container.getGatePassNo() == null)) {
			...
			if (gatePassOpt.isPresent()) {
				...
				gatePass.setContainerPosition(
						ContainerPosition.fromValue(StringUtils.upperCase(container.getContainerPosition())));
				...
				commonGateInOut
						.setRejectReason(StringUtils.upperCase(container.getCommonGateInOut().getRejectReason()));
				commonGateInOut.setGateInStatus(
						TransactionStatus.fromCode(StringUtils.upperCase(container.getAcceptOrReject())));
