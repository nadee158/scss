...
public class ImportExportGateInService {
	...
	public GateInReponse populateGateIn(GateInRequest gateInRequest) {
		...
		/*
		 * if ((gateInRequest.getGatePass1() != null &&
		 * gateInRequest.getGatePass1() > 0) || (gateInRequest.getGatePass2() !=
		 * null && gateInRequest.getGatePass2() > 0)) {
	...
	public GateInReponse saveTestGateInInfo(GateInWriteRequest gateInWriteRequest) {
		...
		log.error("ERROR MESSAGE FROM OPUS SERVICE: " + errorMessage);
		/*
		 * if (StringUtils.isNotEmpty(errorMessage)) { // save it to the db - TO
		 * BE IMPLEMENTED // throw new business exception with constructed
		 * message - there is // an error throw new
