...
public class SolasController {
  ...
  @RequestMapping(value = "/calculatevgm/{directentry}", method = RequestMethod.PUT, produces = MediaType.APPLICATION_JSON_UTF8_VALUE,
      consumes = MediaType.APPLICATION_JSON_VALUE)
  public CustomResponseEntity<ApiResponseObject<?>> calculateVGM(@PathVariable boolean directentry, @RequestBody List<ExportContainer> exportContainers) {
    logger.info("exportContainers.size() " + exportContainers.size());
    System.out.println("directentry "+directentry);
    exportContainers = solasService.calculateTerminalVGM(exportContainers, directentry);
