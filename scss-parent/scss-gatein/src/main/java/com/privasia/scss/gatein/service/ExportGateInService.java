...
public class ExportGateInService {
  ...
  public void saveGateInInfo(GateInWriteRequest gateInWriteRequest, Client gateInClient, SystemUser gateInClerk,
    ...
    gateInWriteRequest.getExportContainers().forEach(exportContainer -> {
      ...
      exports.prepareForInsertFromOpus(gateInClerk, card, gateInClient, scn, hpabBooking, damageCodeRepository);
      exports.setTosServiceType(finalTOSServiceType);
  ...
  public void updateReferReject(GateInWriteRequest gateInWriteRequest, List<ExportContainer> containers) {
    ...
    Predicate byReferRejectID =
        ReferRejectPredicates.byReferRejectID(gateInWriteRequest.getReferRejectDTO().get().getReferRejectID());
    ...
    if (referReject == null) {
      throw new BusinessException(
          "Refer reject to update is not available!" + gateInWriteRequest.getReferRejectDTO().get().getReferRejectID());
    }

    referReject.getReferRejectDetails().forEach(referRejectDetail -> {
      Optional<ExportContainer> optExportContainer =
          containers.stream().filter(exports -> StringUtils.endsWithIgnoreCase(referRejectDetail.getContainerNo(),
              exports.getContainer().getContainerNumber())).findAny();
