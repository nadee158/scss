...
public class ReferRejectService {
	...
	public ReferReject saveReferReject(GateInWriteRequest gateInWriteRequest) {
		...
		if ((!referRejectDTO.getReferRejectDetails().isPresent()
				|| referRejectDTO.getReferRejectDetails().get().isEmpty()))
		...
		if(baseCommonGateInOut.getHpatBooking().isPresent()){
			hpabBooking = hpatBookingRepository
	                  .findOne(baseCommonGateInOut.getHpatBooking().get()).orElseThrow(() -> new ResultsNotFoundException("No HPAB Booking found ! : "+
	                		  baseCommonGateInOut.getHpatBooking().get()));
		}
	...
	@Transactional(value = "transactionManager", propagation = Propagation.REQUIRED, readOnly = true)
	public ReferReject updateReferRejectReference(ReferRejectDTO referRejectDTO, ReferReject referReject, HPABBooking hpabBooking) {
		...
		Card card = cardRepository.findOne(baseCommonGateInOutDTO.getCard())
				.orElseThrow(() -> new AuthenticationServiceException(
		...
		if (!(rejectReasonDTOs == null || rejectReasonDTOs.isEmpty())) {
			...
			rejectReasonDTOs.forEach(referReasonDTO -> {
				...
				ReferReason referReason = referReasonRepository.findOne(referReasonDTO.getReferReasonID())
						.orElseThrow(() -> new ResultsNotFoundException(
								"Provided Refer Reson Not Found " + referReasonDTO.getReferReasonID()));
		...
		referReject.getReferRejectDetails().get().forEach(referRejectDetail -> {
			dto.getReferRejectDetails().get().forEach(referRejectDetailDTO -> {
				if (referRejectDetail.getReferRejectDetailID() == referRejectDetailDTO.getReferRejectDetailID()) {
					...
					referRejectDetail
							.setSupervisorRemarks(StringUtils.upperCase(referRejectDetailDTO.getSupervisorRemarks()));
	...
	public String savePrintReject(long referId, String ipAddress) {
		...
		if (!(referReject == null)) {
			if (!(referReject.getReferRejectDetails().isPresent()
					|| referReject.getReferRejectDetails().get().isEmpty())) {
				...
				if (!(systemUserId == null || systemUserId == 0)) {
					...
					if (referReject.getBaseCommonGateInOut().get().getCard() != null) {
						SmartCardUser cardUser = referReject.getBaseCommonGateInOut().get().getCard()
								.getSmartCardUser();
						if (cardUser != null) {
					...
					for (ReferRejectDetail referRejectDetail : referReject.getReferRejectDetails().get()) {
						if (i == 0) {
							...
							printReject = setRejectReasonsForPrentReject(printReject,
									referRejectDetail.getReferRejectReason().get(), i);
						} else {
							// container 2
