...
public class CardValidationService {
	...
	public boolean validateCardStatus(String cardNo){
		...
		cardOptional.ifPresent(foundCard -> {cardOptional.get();
		...
		/*
		 check the card number in  scss_company comp, scss_card card
		 	if(found){
		 		...
		 		write a switch statement for following validation
		 		if (cardStatus.equals(Card.ACTIVE) && compStatus.equals(Company.ACTIVE)) {
			      ret = RET_OK;
			    } else if (cardStatus.equals(Card.BLACKLIST)) {
			      ret = CARD_ERR_BLACKLIST;
			    } else if (cardStatus.equals(Card.EXPIRED)) {
			      ret = CARD_ERR_EXPIRED;
			    } else if (cardStatus.equals(Card.SUSPENDED)) {
			      ret = CARD_ERR_SUSPENDED;
			    } else if (cardStatus.equals(Card.TERMINATED)) {
			      ret = CARD_ERR_TERMINATED;
			    } else if (cardStatus.equals(Card.PENDING)) {
			      ret = CARD_ERR_PENDING;
			    } else if (cardStatus.equals(Card.NOT_ISSUED)) {
			      ret = CARD_ERR_NOT_ISSUED;
			    } else if (compStatus.equals(Company.SUSPENDED)) {
			      ret = COMP_ERR_SUSPENDED;
			    } else if (compStatus.equals(Company.TERMINATED)) {
			      ret = COMP_ERR_TERMINATED;
			    } else {
			      ret = RET_UNKNOWN;
			    }
		 		
		 		return the object with card status and isMC true
		 	}else{
		 		return null object
		 	}
		 */
