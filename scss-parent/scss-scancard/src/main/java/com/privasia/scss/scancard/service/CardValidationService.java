...
public class CardValidationService {
	...
	public CardValidationDto validateCard(String cardNo) {
		...
		if (isValidCardNo) {
			...
			return new CardValidationDto(null, false, ScanCardConstant.CARD_ERR_NOT_VALID_LENGTH, null);
		}
		throw new ResultsNotFoundException("Requested Card was not found!");
	}
	...
	public CardValidationDto validateCompanyStatus(long cardId, CompanyStatus companyStatus) {
		...
		case ACTIVE:
			// ret = RET_OK;
			cardValidation = new CardValidationDto(cardId, true, ScanCardConstant.RET_OK,
					CompanyStatus.ACTIVE.getValue());
			break;
		case CREATED:
			// RET_UNKNOWN;
			cardValidation = new CardValidationDto(cardId, false, ScanCardConstant.RET_UNKNOWN,
					CompanyStatus.CREATED.getValue());
			break;
		case SUSPENDED:
		...
		case UPDATED:
			// RET_UNKNOWN;
			cardValidation = new CardValidationDto(cardId, false, ScanCardConstant.RET_UNKNOWN,
					CompanyStatus.UPDATED.getValue());
			break;
		default:
			// RET_UNKNOWN;
