...
public final class KioskBoothRightsPredicates {
	...
	public static Predicate KioskBoothStatus(String Status) {
		if (StringUtils.isEmpty(Status) || KioskLockStatus.fromCode(Status) == null) {
	...
	public static Predicate KioskBoothStatus(KioskLockStatus status) {
		if (status == null) {
			return QKioskBoothRights.kioskBoothRights.isNull();
		} else {
			return QKioskBoothRights.kioskBoothRights.kioskLockStatus.eq(status);
		}
	}
