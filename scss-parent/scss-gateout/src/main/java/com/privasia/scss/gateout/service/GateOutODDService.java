...
public class GateOutODDService {
  ...
  public ContainerValidationInfo validateODDContainers(ContainerValidationInfo containerValidationInfo) {
    ...
    containerValidationInfo
        .setContainerNo1Status(cosmosODDRepository.validateODDContainer(containerValidationInfo.getContainerNo1()));

    if (StringUtils.isNotEmpty(containerValidationInfo.getContainerNo2())) {
      containerValidationInfo
          .setContainerNo2Status(cosmosODDRepository.validateODDContainer(containerValidationInfo.getContainerNo2()));
    }
  ...
  public List<WHODD> fetchInProgressTransaction(Long cardID) {
    ...
    Optional<List<WHODD>> optionalODDList =
        oddRepository.findByCardIDAndEirStatus(cardID, TransactionStatus.INPROGRESS);
    ...
    if (optionalODDList.isPresent()) {
      ...
      transactionList.forEach(whODD -> {
        ...
        /*
         * if(oddDTO.getContainer02() != null){ Optional<ODDLocation> optionalLocation =
         * oddLocationRepository.findOne(oddDTO.getContainer02().getLocation());
         * if(optionalLocation.isPresent()){
