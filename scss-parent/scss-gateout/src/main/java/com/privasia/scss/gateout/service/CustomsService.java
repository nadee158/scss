...
public class CustomsService {
	...
	public String updateCustomsODD(CustomsDTO customsDTO) {
		...
		Predicate byWHoddIDList = ODDPredicates.byWHoddIDList(Arrays.asList(customsDTO.getOddIdSeq01().orElse(null),
				customsDTO.getOddIdSeq02().orElse(null)));
		...
		if ((whODDList != null && whODDList.iterator().hasNext())) {
			...
			whODDList.forEach(whODD -> {
				...
				if (customs.getContainer01() == null) {
					...
					customs.setContainer02(constructCustomContainer(whODD.getContainer02(), whODD.getImpExpFlag()));
				}
	...
	public String updateCustomsImport(CustomsDTO customsDTO) {
		...
		Predicate byGatePassIDList = GatePassPredicates.byGatePassIDList(Arrays.asList(customsDTO.getGatePassIDSeq01().orElse(null),
	...
	public String updateCustomsExport(CustomsDTO customsDTO) {
		...
		Predicate byExportsIDList = ExportsPredicates.byExportsIDList(Arrays.asList(customsDTO.getExportIDSeq01().orElse(null),
	...
	private CustomContainer constructCustomContainer(ODDContainerDetails oddContainerDetails, ImpExpFlagStatus impExpFlag) {
		
		if(oddContainerDetails !=null && (StringUtils.equalsIgnoreCase(oddContainerDetails.getFullOrEmpty().getValue(), 
						ContainerFullEmptyType.FULL.getValue()))){
			
			if((StringUtils.equalsIgnoreCase(impExpFlag.getValue(), ImpExpFlagStatus.EXPORT.getValue()))
					&& (StringUtils.equalsIgnoreCase(oddContainerDetails.getOddStatus().getValue(), TransactionStatus.APPROVED.getValue()))){
				return null;
			}
			CustomContainer customContainer = new CustomContainer();
