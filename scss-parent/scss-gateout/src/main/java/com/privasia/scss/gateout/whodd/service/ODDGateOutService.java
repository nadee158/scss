...
public class ODDGateOutService {
  ...
  public GateOutReponse populateGateOut(GateOutRequest gateOutRequest, GateOutReponse gateOutReponse) {
    ...
    Optional<List<WHODD>> optionalODDList =
        oddRepository.findByCardIDAndEirStatus(gateOutRequest.getCardID(), TransactionStatus.INPROGRESS);
    ...
    if (optionalODDList.isPresent()) {
      ...
      transactionList.forEach(whODD -> {
        ...
        if (oddDTO.getContainer01() != null) {
          Optional<ODDLocation> optionalLocation =
              oddLocationRepository.findOne(oddDTO.getContainer01().getLocation().getOddCode());
        ...
        if (oddDTO.getContainer02() != null) {
          Optional<ODDLocation> optionalLocation =
              oddLocationRepository.findOne(oddDTO.getContainer02().getLocation().getOddCode());
          if (optionalLocation.isPresent()) {
            ODDLocationDTO locationDTO = modelMapper.map(optionalLocation.get(), ODDLocationDTO.class);
      ...
      boolean imports = optionalODDList.get().stream().filter(
          whODD -> StringUtils.equalsIgnoreCase(whODD.getImpExpFlag().getValue(), ImpExpFlagStatus.IMPORT.getValue()))
          .findAny().isPresent();

      boolean exports = optionalODDList.get().stream().filter(
          whODD -> StringUtils.equalsIgnoreCase(whODD.getImpExpFlag().getValue(), ImpExpFlagStatus.EXPORT.getValue()))
          .findAny().isPresent();
  ...
  public GateOutReponse saveODDGateOutInFo(GateOutWriteRequest gateOutWriteRequest) {
    ...
    gateOutReponse.getWhODDContainers().forEach(whODDdto -> {
      ...
      WHODD whODD = optODD
          .orElseThrow(() -> new ResultsNotFoundException("ODD Transaction Not Found : " + whODDdto.getOddIdSeq()));
            ...
            if (StringUtils.isNotBlank(gateOutClient.getType().getValue())) {
              if (ClientType.GATE_IN.getValue()
                ...
                throw new BusinessException("Gate out transaction not allowed for gate in lane "
                    + gateOutClient.getDescription() + ". Please rescan the card at gate out lane and try again.");
              }
            }
          }
      ...
      if (whODD.getContainer01().getHdbsBkgDetailNo().isPresent()) {
        ...
        Optional<HDBSBkgDetail> optHDBSBookingDetail =
            hdbsBookingDetailRepository.findOne(whODD.getContainer01().getHdbsBkgDetailNo().get().getHdbsBKGDetailID());
        HDBSBkgDetail hdbsBookingDetail =
  ...
  public void updateODDReference(FileDTO fileDTO) {
    ...
    Optional<List<WHODD>> oddOptList =
        oddRepository.findByOddIdSeqIn(Arrays.asList(fileDTO.getOddSeq1().orElse(0l), fileDTO.getOddSeq2().orElse(0l)));
    ...
    if (oddOptList.isPresent() && !oddOptList.get().isEmpty()) {
      ...
      throw new BusinessException("Invalid WhODD ID to update file reference : " + fileDTO.getOddSeq1().orElse(null)
          + " / " + fileDTO.getOddSeq2().orElse(null));
    }
  }
  ...
  public void setRejectReason(ImpExpFlagStatus impExpFlag, ODDContainerDetails container,
    ...
    switch (impExpFlag) {
      case IMPORT:
        Optional<ODDImportReason> optODDImportReason =
            oddImportReasonRepository.findOne(containerDTO.getRejectionReasonID());
      ...
      case EXPORT:
        ...
        Optional<ODDExportReason> optODDExportReason =
            oddExportReasonRepository.findOne(containerDTO.getRejectionReasonID());
