...
public class CosmosImportRepository {
  ...
  /*
   * @Value("${import.getSealInfo}") private String queryGetSealInfo;
   */
  ...
  private ImportContainer mapToImportContainer(ImportContainer importContainer, ResultSet rs) {
    ...
    try {
      ...
      throw new BusinessException(
          "Error cound while fetching container in cosmos " + importContainer.getContainer().getContainerNumber());
    }
  ...
  /*
   * @Transactional(value = "as400TransactionManager", propagation = Propagation.REQUIRED, readOnly
   * = true) public Optional<List<SealInfo>> getSealInfo(String handingID) {
   * 
   * handingID = StringUtils.upperCase(handingID); return jdbcTemplate.query(queryGetSealInfo, new
   * Object[] { handingID }, this::extractSealInfo);
   * 
   * }
   * 
   * private Optional<List<SealInfo>> extractSealInfo(ResultSet rs) throws SQLException,
   * DataAccessException {
   * 
   * SealInfo sealInfo = null; List<SealInfo> sealInfoList = null; if (rs != null) { sealInfoList =
   * new ArrayList<>(); while (rs.next()) { sealInfo = new SealInfo();
   * sealInfo.setSealOrigin(TextString.format(rs.getString("slor2k")));
   * sealInfo.setSealType(TextString.format(rs.getString("sltp2k")));
   * sealInfo.setSealNo(TextString.format(rs.getString("seal2k"))); sealInfoList.add(sealInfo); } }
   * 
  ...
  /*
   * @Transactional(value = "as400TransactionManager", propagation = Propagation.REQUIRED, readOnly
   * = true) public boolean checkLaden(String containerNo) {
   * 
   * containerNo = StringUtils.upperCase(containerNo); return jdbcTemplate.query(queryGetSealInfo,
   * new Object[] { containerNo }, this::verifyLaden);
   * 
   * }
   * 
   * private boolean verifyLaden(ResultSet rs) throws SQLException {
   * 
   * if (rs.next()) { if (rs.getString("cnbt03") != null && rs.getString("cnbt03").equals("E")) { //
   * return GatePassErrMsg.GATE_PASS_OK; return true; } } // return
   * GatePassErrMsg.GATE_PASS_NO_PREARRIVAL; return false; }
   */
  ...
  public boolean isOGABlock(ImportContainer importContainer) {
    ...
    String containerNo = StringUtils.upperCase(importContainer.getContainer().getContainerNumber());
    try {
      return jdbcTemplate.queryForObject(queryIsOGABlock, new Object[] {containerNo},
          (rs, i) -> extractOGAOrInternalBlock(rs));
    } catch (EmptyResultDataAccessException e) {
      return false;
    }
  ...
  private ImportContainer extractDsoSealNumbers(ImportContainer importContainer, ResultSet rs, int rowNum)
    ...
    try {
      ...
      log.error("fetchDsoSealNo extractDsoSealNumbers " + e.getMessage());
      e.printStackTrace();
      throw new BusinessException(
          "Error cound while extracting Dso Seal Numbers " + importContainer.getContainer().getContainerNumber());
    }
    return importContainer;
  }
