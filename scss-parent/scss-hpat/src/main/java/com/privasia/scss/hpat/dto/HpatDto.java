...
public class HpatDto implements Serializable {
	...
	public HpatDto(HPATBooking b) {
		...
		b.getHpatBookingDetails().forEach(detail -> setGatePassAndContainDetail(detail));
		

	}

	public void setGatePassAndContainDetail(HPATBookingDetail hpabBookingDetail) {

		if (StringUtils.isEmpty(this.expContainer01)) {
			this.expContainer01 = hpabBookingDetail.getContainerNumber();
		} else {
			this.expContainer02 = hpabBookingDetail.getContainerNumber();
		}

		if (StringUtils.equals(BookingType.IMPORT.getValue(), hpabBookingDetail.getBookingType().getValue())) {

			if (StringUtils.isEmpty(this.impGatePass01)) {
				this.impGatePass01 = hpabBookingDetail.getImpGatePassNumber();

			} else {
				this.impGatePass02 = hpabBookingDetail.getImpGatePassNumber();
		...
		if (StringUtils.equals(BookingType.IMPORT_ITT.getValue(), hpabBookingDetail.getBookingType().getValue())) {
			...
			if (StringUtils.isEmpty(this.ittGatePass01)) {
				this.ittGatePass01 = hpabBookingDetail.getImpGatePassNumber();
